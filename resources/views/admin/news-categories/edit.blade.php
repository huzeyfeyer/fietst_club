            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.news-categories.update', $newsCategory) }}" method="POST">
                        ...
                        {{-- Huidige Slug (read-only ter informatie, wordt automatisch bijgewerkt als naam wijzigt) --}}
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Huidige Slug') }}</label>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $newsCategory->slug }}</p>
                        </div>
