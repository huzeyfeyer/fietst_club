            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col md:flex-row justify-between md:items-center mb-6">
                        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200 mb-4 md:mb-0">
                            @if ($activeCategory)
                                Nieuws in categorie: <span class="text-indigo-600 dark:text-indigo-400">{{ $activeCategory->name }}</span>
                            @else
                                Laatste Nieuws
                            @endif
                        </h1>
                        @can('create', App\Models\News::class)
                            <a href="{{ route('news.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 self-start md:self-auto">
                                Nieuw bericht
                            </a>
                        @endcan
                    </div>
                    ...
                    <div class="space-y-8">
                        @forelse($news as $item)
                            <article class="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-lg flex flex-col md:flex-row md:space-x-6">
                                @if ($item->image_path)
                                ...
                                <div class="flex-grow {{ $item->image_path ? 'md:w-2/3 lg:w-3/4' : 'w-full' }}">
                                    <header class="mb-3">
                                        <h2 class="text-2xl font-semibold hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                    ...
                                    <a href="{{ route('news.show', $item) }}" class="text-blue-600 dark:text-blue-400 hover:underline font-semibold">
                                        Lees meer →
                                    </a>
                                </div>
                            </article>
