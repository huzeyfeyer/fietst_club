            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('news.update', $news) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="image" class="block text-gray-700 dark:text-gray-300 font-bold mb-2">Afbeelding</label>
                            @if ($news->image_path)
                                <div class="mt-2 mb-2">
                                    <img src="{{ asset('storage/' . $news->image_path) }}" alt="Huidige afbeelding" class="max-w-xs h-auto rounded-md shadow-md">
                                    <label for="remove_image" class="inline-flex items-center mt-2">
                                        <input type="checkbox" id="remove_image" name="remove_image" value="1" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800">
                                        <span class="ms-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Huidige afbeelding verwijderen') }}</span>
                                    </label>
                                </div>
                            @endif
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">PNG, JPG of GIF (MAX. 2MB). Laat leeg om huidige afbeelding te behouden.</p>
                             @error('image')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @forelse ($categories as $category)
                                    <label for="category_{{ $category->id }}" class="flex items-center p-2 border border-gray-300 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                                        <input type="checkbox" name="categories[]" id="category_{{ $category->id }}" value="{{ $category->id }}"
                                               {{ (is_array(old('categories')) && in_array($category->id, old('categories'))) || ( !old('categories') && in_array($category->id, $selectedCategories)) ? 'checked' : '' }}
                        ...
                        <div class="flex justify-between items-center mt-6">
                            <a href="{{ route('news.show', $news) }}" class="text-gray-600 dark:text-gray-400 hover:underline">
                                Annuleren
                            </a>
